...
Phase 2 candidate construction preserves Unicode words, including Hindi and Marathi combining marks, rather than dropping them during query construction. IDs, dates and compact numeric/unit values are also added as literal candidate-recall hints. They neither bypass permissions/lifecycle filters nor establish that a result set is complete. Their live PostgreSQL behavior still requires the pending disposable-database validation and human-reviewed multilingual examples.
...
- Model-specific tokenizer accounting. The serving code now uses a conservative tokenizer-independent estimate and reserves explicit instruction, history and requested-output space; it is not a substitute for a model tokenizer measurement.
